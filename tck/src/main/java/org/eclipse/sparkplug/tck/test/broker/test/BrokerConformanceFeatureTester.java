...
public class BrokerConformanceFeatureTester {
    ...
    public BrokerConformanceFeatureTester(final @NotNull String host,
    ...
    public @Nullable Mqtt3ConnAck testConnectWithWill() {
        ...
        try {
            final Mqtt3ConnAck connAck = client.toBlocking()
                    .connectWith()
                    .willPublish(will)
                    .send();
    ...
    public @NotNull SharedSubscriptionTestResult testSharedSubscription() {
        ...
        Logger.debug(" .        COMPLIANCE CHECK: Subscribing first subscriber to shared topic {} with qos {}", sharedTopic, maxQos);
        ...
        Logger.debug(" .        COMPLIANCE CHECK: Subscribing second subscriber to shared topic {} with qos {}", sharedTopic, maxQos);
    ...
    public @NotNull ComplianceTestResult testRetain() {
        ...
        try {
            Logger.debug(" .        COMPLIANCE CHECK: Publishing retained message '{}' to topic {} with qos {}", "RETAIN", topic, maxQos);
            publisher.toBlocking().publishWith()
                    .topic(topic)
                    .qos(maxQos)
                    .retain(true)
        ...
        final ComplianceTestResult complianceTestResult = countDownLatch.getCount() == 0 ? ComplianceTestResult.OK : ComplianceTestResult.TIME_OUT;
    ...
    public @NotNull QosTestResult testQos(final @NotNull MqttQos qos, final int tries) {
        ...
        Logger.debug(" .        COMPLIANCE CHECK: Result of testing qos {}: Received {} / {} publishes", qos, totalReceived, tries);
    ...
    public @NotNull PayloadTestResults testPayloadSize(final int maxSize) {
        ...
        if (maxTestSuccess) {
            ...
            Logger.debug(" .        COMPLIANCE CHECK: Result of testing max. payload size: {} bytes",
                    mid);
    ...
    private boolean testPayload(final @NotNull String topic,
        ...
        try {
            publisher.toBlocking().connect();
            Logger.debug(" .        COMPLIANCE CHECK: Publishing payload with {} bytes to topic {} with qos {}", payloadSize, topic, maxQos);
        ...
        try {
            ...
            if (!receive.isPresent()) {
                disconnectIfConnected(publisher, subscriber);
                Logger.debug(" .        COMPLIANCE CHECK: Timed out while waiting for publish with {} bytes", currentPayload.getBytes().length);
                ...
                disconnectIfConnected(publisher, subscriber);
                Logger.debug(" .        COMPLIANCE CHECK: Received wrong payload for publish with {} bytes", currentPayload.getBytes().length);
                testResults.add(new Tuple<>(payloadSize, ComplianceTestResult.WRONG_PAYLOAD));
                disconnectIfConnected(publisher, subscriber);
                return false;
            }
        } catch (InterruptedException e) {
            Logger.error("Interrupted while waiting for subscriber to receive payload with length {} bytes", currentPayload.getBytes().length, e);
    ...
    private boolean testTopic(final @NotNull List<Tuple<Integer, ComplianceTestResult>> testResults,
        ...
        try {
            Logger.debug(" .        COMPLIANCE CHECK: Subscribing to topic with {} bytes with qos {}", topicSize, maxQos);
            subscriber.toBlocking().subscribe(subscribe);
        } catch (final Exception ex) {
            Logger.error("Failed to subscribe to topic with a length of {} bytes", currentTopicName.getBytes().length, ex);
        ...
        try {
            publisher.toBlocking().connect();
            Logger.debug(" .        COMPLIANCE CHECK: Publishing to topic with {} bytes with qos {}", topicSize, maxQos);
            publisher.toBlocking().publish(publish);
        } catch (final Exception ex) {
        ...
        try {
            final Optional<Mqtt3Publish> receive = publishes.receive(timeOut, TimeUnit.SECONDS);
            if (!receive.isPresent()) {
                Logger.debug(" .        COMPLIANCE CHECK: Timed out while waiting to receive a publish from topic {}", currentTopicName);
                ...
                Logger.debug(" .        COMPLIANCE CHECK: Received wrong payload for publish to topic {}", currentTopicName);
                testResults.add(new Tuple<>(topicSize, ComplianceTestResult.WRONG_PAYLOAD));
                disconnectIfConnected(subscriber, publisher);
                return false;
            }
        } catch (InterruptedException e) {
            Logger.error("Interrupted while waiting to receive publish to topic with {} bytes", currentTopicName.getBytes().length, e);
    ...
    public @NotNull ClientIdLengthTestResults testClientIdLength() {
        ...
        if (maxClientIdSuccess) {
            ...
            Logger.debug(" .        COMPLIANCE CHECK: Result of testing max. client identifier length: {} bytes", MAX_CLIENT_ID_LENGTH);
            ...
            Logger.debug(" .        COMPLIANCE CHECK: Setting max. client identifier length to {} bytes for further tests", mid);
    ...
    private boolean testClientIdLength(final @NotNull List<Tuple<Integer, String>> connectResults,
        ...
        try {
            ...
            if (connAck.getReturnCode() != Mqtt3ConnAckReturnCode.SUCCESS) {
                Logger.debug(" .        COMPLIANCE CHECK: Received non-successful return code {}", connAck.getReturnCode());
                return false;
            }
            ...
            Logger.error("Connect with client id length {} bytes",
                    currClient.getConfig().getClientIdentifier()
                            .map(id -> id.toString().getBytes().length).orElse(0), ex);
    ...
    private @NotNull ComplianceTestResult testWildcard(final String subscribeWildcardTopic, final String publishTopic) {
        ...
        try {
            Logger.debug(" .        COMPLIANCE CHECK: Subscribing to wildcard topic {} with qos {}", subscribeToTopic, maxQos);
            subscriber.toAsync().subscribeWith()
        ...
        try {
            Logger.debug(" .        COMPLIANCE CHECK: Publishing to wildcard topic {} with qos {}", publishTopic, maxQos);
            publisher.toBlocking().publishWith()
        ...
        try {
            ...
            Logger.error("Interrupted while subscription to {} receives publish to {}",
                    subscribeToTopic,
                    publishToTopic, e);
        }
        ...
        disconnectIfConnected(publisher, subscriber);
        final ComplianceTestResult complianceTestResult = countDownLatch.getCount() == 0 ? ComplianceTestResult.OK : ComplianceTestResult.TIME_OUT;
        Logger.debug(" .        COMPLIANCE CHECK: Result of testing wildcard topic {}: {}", subscribeWildcardTopic, complianceTestResult);
    ...
    public @NotNull AsciiCharsInClientIdTestResults testAsciiCharsInClientId() {
        ...
        if (allSuccess) {
            ...
            Logger.debug(" .        COMPLIANCE CHECK: Result of testing ascii character in client identifier: Unsupported characters {}", connectResults.toString());
            return new AsciiCharsInClientIdTestResults(connectResults);
        }
    }
    ...
    private void testAsciiChar(final @NotNull List<Tuple<Character, String>> connectResults,
        ...
        try {
            ...
            Logger.debug(" .        COMPLIANCE CHECK: Could not connect client identifier with ascii char '{}'", asciiChar, ex);
