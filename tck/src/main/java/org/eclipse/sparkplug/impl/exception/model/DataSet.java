...
import org.eclipse.sparkplug.impl.exception.SparkplugErrorCode;
import org.eclipse.sparkplug.impl.exception.SparkplugException;
import org.eclipse.sparkplug.impl.exception.json.DataSetDeserializer;
	...
	public static class DataSetBuilder {
		...
		public void validate() throws SparkplugException {
			if (columnNames.size() != numOfColumns) {
				throw new SparkplugException(SparkplugErrorCode.INVALID_ARGUMENT,
						"Invalid number of columns in data set column names: " + columnNames.size() + " vs expected "
								+ numOfColumns);
			}
			if (types.size() != numOfColumns) {
				throw new SparkplugException(SparkplugErrorCode.INVALID_ARGUMENT,
						"Invalid number of columns in data set types: " + types.size() + " vs expected: "
								+ numOfColumns);
			}
			for (int i = 0; i < types.size(); i++) {
				for (Row row : rows) {
					...
					if (values.size() != numOfColumns) {
						throw new SparkplugException(SparkplugErrorCode.INVALID_ARGUMENT,
								"Invalid number of columns in data set row: " + values.size() + " vs expected: "
										+ numOfColumns);
					}
