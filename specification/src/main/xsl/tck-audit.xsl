...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt"
    ...
    <xsl:template match="xi:include" mode="merge">
        <xsl:variable name="fileName">en/<xsl:value-of select="@href"/>
        </xsl:variable>
    ...
    <xsl:template match="article | book" mode="createAuditFile">
        ...
        <specification xmlns="http://jboss.com/products/weld/tck/audit"
                       ...
                       xsi:schemaLocation="http://jboss.com/products/weld/tck/audit" name="Sparkplug 3.0.0-SNAPSHOT"
                       version="3.0.0-SNAPSHOT" id="sparkplug" generateSectionIds="true">
    ...
    <xsl:template match="section | upperalpha" mode="createAuditFile">
        <xsl:call-template name="check-section-id">
            ...
            <xsl:with-param name="sectionNum">
        ...
        <section>
            ...
            <xsl:variable name="sectionIdConstant">
                <xsl:call-template name="section-id-to-constant">
                    <xsl:with-param name="sectionId" select="@xml:id"/>
                </xsl:call-template>
            </xsl:variable>
            <xsl:comment>
                <xsl:text> </xsl:text><xsl:value-of select="@sectionNum"/><xsl:text> - </xsl:text><xsl:value-of
                    select="$sectionIdConstant"/><xsl:text> </xsl:text>
            </xsl:comment>
    ...
    <xsl:template match="*[starts-with(@role, 'tck')]" mode="createAuditFile">
        <xsl:variable name="normalized">
            <xsl:value-of select="normalize-space(translate(., '’', $apos))"/>
        </xsl:variable>
        <xsl:variable name="firstWord">
            <xsl:value-of select="substring-before(concat($normalized, ' '), ' ')"/>
        </xsl:variable>
        ...
        <xsl:variable name="assertionText">
            ...
            <xsl:choose>
                <xsl:when
                        test="string-length(translate($firstWord, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ-', '')) &lt; string-length($firstWord)">
                    <xsl:value-of select="$normalized"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of
                            select="concat(translate(substring($normalized, 1, 1), 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), substring($normalized, 2))"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
    ...
    <xsl:template name="check-section-id">
        ...
        <xsl:if test="starts-with($sectionId, '_')">
            <xsl:message terminate="yes">
                Error: section
                <xsl:value-of select="$sectionNum"/><xsl:text> - </xsl:text><xsl:value-of select="$sectionId"/> seems to
                be automatically generated: it starts with an underscore.
            </xsl:message>
        </xsl:if>
    </xsl:template>
